...
open! Base
open! Stdio
module I = Ocaml_intrinsics.Float.Unboxed
...
let%expect_test "round" =
  List.iter args ~f:(fun x ->
    ...
    let res = I.round_half_to_even (Float_u.of_float x) in
    printf "[Base] round even (current) %.19g = %.19g" x (Float_u.to_float res);
    if same_bits (Float_u.to_float res) base_res
    then printf "\n"
  ...
  List.iter args ~f:(fun x ->
    printf
      "round even (current) %.19g = %.19g\n"
      x
      (Float_u.to_float (I.round_half_to_even (Float_u.of_float x))));
  [%expect
    {|
  ...
  List.iter args ~f:(fun x ->
    printf
      "round nearest %.19g = %.19g\n"
      x
      (Float_u.to_float (I.round_nearest (Float_u.of_float x))));
  [%expect
    {|
  ...
  List.iter args ~f:(fun x ->
    printf
      "round up %.19g = %.19g\n"
      x
      (Float_u.to_float (I.round_up (Float_u.of_float x))));
  [%expect
    {|
    round up inf = inf
  ...
  List.iter args ~f:(fun x ->
    printf
      "round down %.19g = %.19g\n"
      x
      (Float_u.to_float (I.round_down (Float_u.of_float x))));
  [%expect
    {|
    round down inf = inf
  ...
  List.iter args ~f:(fun x ->
    printf
      "round to zero %.19g = %.19g\n"
      x
      (Float_u.to_float (I.round_towards_zero (Float_u.of_float x))));
  [%expect
    {|
...
let%expect_test "bit_of_float" =
  List.iter args ~f:(fun x ->
    ...
    then (
      let y = Ocaml_intrinsics.Int64.bits_of_float x in
      let z = Ocaml_intrinsics.Int64.float_of_bits y in
      let check = if same_bits x z then "" else " FAIL " in
      printf "bits_of_float %.19g = %Ld -> %.19g%s\n" x y z check));
  [%expect
    {|
